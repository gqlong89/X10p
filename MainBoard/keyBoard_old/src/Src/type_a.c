...
unsigned char TypeA_Request(unsigned char *pTagType) {
    unsigned char xdata result, send_buff[1], rece_buff[2];
    unsigned int xdata rece_bitlen = 0;
...
//    unsigned char xdata result, send_buff[1], rece_buff[2];
//    unsigned int xdata rece_bitlen;
//    Clear_BitMask(TxModeReg, 0x80);//关闭TX CRC
//    Clear_BitMask(RxModeReg, 0x80);//关闭RX CRC
//    Set_BitMask(RxModeReg, 0x08);//关闭位接收
...
unsigned char TypeA_Anticollision(unsigned char selcode, unsigned char *pSnr) {
    unsigned char xdata result, send_buff[7], rece_buff[5], CollPos, NVB, Row, Col, Bit_Framing, Send_Len, Snr[5], i, Rece_Len;
    unsigned int xdata rece_bitlen = 0;
...
unsigned char TypeA_Select(unsigned char selcode, unsigned char *pSnr, unsigned char *pSak) {
    unsigned char xdata result, i, send_buff[7], rece_buff[5];
    unsigned int xdata rece_bitlen;
...
unsigned char TypeA_Halt(void) {
    unsigned char xdata result, send_buff[2], rece_buff[1];
    unsigned int xdata rece_bitlen;
    send_buff[0] = 0x50;
    send_buff[1] = 0x00;
...
{
	unsigned char xdata send_buff[12],rece_buff[1],result;
	unsigned int xdata rece_bitlen;
	if(mode==0x0)
...
/*****************************************************************************************/
 ...
 {
  unsigned char xdata block_data[16],result;
	block_data[0]=buff[3];
	block_data[1]=buff[2];
	block_data[2]=buff[1];
	block_data[3]=buff[0];
...
{	
	unsigned char xdata send_buff[2],result;
	unsigned int xdata rece_bitlen;
	Pcd_SetTimer(1);
...
{	
	unsigned char xdata result,send_buff[16],rece_buff[1];
	unsigned int xdata rece_bitlen;
	Pcd_SetTimer(1);
    ...
    Debug_Log("rece_bitlen");
    Debug_Hex((unsigned char *)&rece_bitlen, 4);
    Debug_Log("\r\n");
    
    Debug_Log("rece_buff[0]");
    Debug_Hex(&rece_buff[0], 1);
    Debug_Log("\r\n");
	#endif
...
{	
	unsigned char xdata  result,send_buff[4],rece_buff[1];
	unsigned int  xdata rece_bitlen;
	Pcd_SetTimer(5);
	send_buff[0]=0xc2;//
	send_buff[1]=block;//块地址
