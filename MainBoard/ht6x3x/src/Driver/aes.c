...
#if 0
//******************************************************************
//! \brief  	HT_AES_Encrypt
//! \param  	ESKeyMode			枚举变量，选择加密模式
//!  				KEY_Mode_128bits
//!					KEY_Mode_192bits
//!					KEY_Mode_256bits
//! \param		pKeyAddr  			uint32_t *型变量，其指向对象存储AES key : pKeyAddr[0]为高位
//! \param		pIntDataAddr  		uint32_t *型变量，其指向对象存储待加密数据（明文）: pIntDataAddr[0]为低位
//! \retval
//! \note
//******************************************************************
void HT_AES_Encrypt(AESKeyMode_TypeDef AESKeyMode, uint32_t *pKeyAddr, uint32_t *pIntDataAddr)
{
	...
	//待加密数据（明文）写入寄存器
	HT_AES->AESINLL = *pIntDataAddr++;
	HT_AES->AESINML = *pIntDataAddr++;
	HT_AES->AESINHM = *pIntDataAddr++;
	HT_AES->AESINHH = *pIntDataAddr;

	//启动AES加密
	HT_AES->AESSTR = 0x8329;
}



//******************************************************************
//! \brief  	HT_AES_Decrypt
//! \param  	AESKeyMode			枚举变量，选择解密模式
//!  				KEY_Mode_128bits
//!					KEY_Mode_192bits
//!					KEY_Mode_256bits
//! \param		pKeyAddr  			uint32_t *型变量，其指向对象存储AES key : pKeyAddr[0]为高位
//! \param		pIntDataAddr  		uint32_t *型变量，其指向对象存储待解密数据（密文）: pIntDataAddr[0]为低位
//! \retval
//! \note
//******************************************************************
...
//******************************************************************
//! \brief  	HT_GHASH_Multiply
//! \param  	pIntDataAddr1		uint32_t *型变量，其指向对象存储乘数1数据 : pIntDataAddr1[0]为低位
//! \param		pIntDataAddr2  		uint32_t *型变量，其指向对象存储乘数2数据 : pIntDataAddr2[0]为低位
//! \retval
//! \note		GHASH 乘法运算模块
//******************************************************************
...
//******************************************************************
//! \brief  	HT_AES_GHASH_RAND_ITConfig
//! \param  	ITEn       		AES/GHASH/RAND中断设置位，可以为以下参数或其组合
//!  				 			@arg AES_IE
//!								@arg GHASH_IE
//!					 			@arg RAND_IE
//! \param		NewState   		ENABLE： 使能中断
//! 							DISABLE：关闭中断
//! \retval
//! \note		使能或者关闭AES/GHASH/RAND中断
//******************************************************************
void HT_AES_GHASH_RAND_ITConfig(AES_ITEnTypeDef ITEn, FunctionalState NewState)
{
    ...
    if (NewState != DISABLE){
		//使能AES/GHASH/RAND中断
        HT_GHASH->AESGHASHIE |= (uint32_t)ITEn;
    }else{
		 //关闭AES/GHASH/RAND中断
        HT_GHASH->AESGHASHIE &= ~(uint32_t)ITEn;
    }
...
//******************************************************************
//! \brief  	HT_AES_GHASH_RAND_ITFlagStatusGet
//! \param  	ITFlag       	AES/GHASH/RAND中断标志位
//!  				 			@arg AES_IE
//!								@arg GHASH_IE
//!					 			@arg RAND_IE
//! \retval		ITStatus   		SET： 相应中断标志产生
//! 							RESET：相应中断标志未产生
//! \note		获取相应AES/GHASH/RAND中断标志状态
//******************************************************************
ITStatus HT_AES_GHASH_RAND_ITFlagStatusGet(AES_ITFlagTypeDef ITFlag)
{
    //assert_param
    if (HT_GHASH->AESGHASHIF & ITFlag){
		//AES/GHASH/RAND Interrupt Flag is set
        return SET;
    } else {
		//AES/GHASH/RAND Interrupt Flag is reset
        return RESET;
    }
}
...
//******************************************************************
//! \brief  	HT_AES_GHASH_RAND_ClearITPendingBit
//! \param  	ITFlag       	想要清除的某个RTC中断标志
//!  				 			@arg AES_IE
//!								@arg GHASH_IE
//!					 			@arg RAND_IE
//! \retval
//! \note		清除AES/GHASH/RAND中断标志
//******************************************************************
void HT_AES_GHASH_RAND_ClearITPendingBit(AES_ITFlagTypeDef ITFlag)
{
	//Clear AES/GHASH/RAND Interrupt Flag
    HT_GHASH->AESGHASHIF  &= ~((uint32_t)ITFlag);

}


//******************************************************************
//! \brief  	HT_AES_Busy_StatusGet
//! \retval		ITStatus   		SET： busy忙碌
//! 							RESET：busy空闲
//! \note		获取相应AES/GHASH/RAND中BUSY状态
//******************************************************************
ITStatus HT_AES_Busy_StatusGet(void)
{
    //返回AES busy status
    return (ITStatus)HT_AES->AESFLG;
}

ITStatus HT_GHASH_Busy_StatusGet(void)
{
	 //返回GHASHCH乘法 busy status
    return (ITStatus)HT_GHASH->GHASHFLG;
}


//******************************************************************
//! \brief  	HT_AES_Xor128bits
//! \param		pIntDataAddr1		uint32_t *型变量，其指向对象存储异或前数据1 : pIntDataAddr1[0]为低位
//! \param		pIntDataAddr2  		uint32_t *型变量，其指向对象存储异或前数据2:  pIntDataAddr2[0]为低位
//! \retval		pOutDataAddr  	uint32_t *型变量，其指向对象存储异或后数据 : pOutDataAddr[0]为低位
//! \note		 128bits 异或函数
//******************************************************************
...
//******************************************************************
//! \brief  	AES_encrypt_hw
//! \param		in				明文数据
//! \param		out  			已加密密文
//! \param		aes				秘钥
//! \retval
//! \note		ECB 128加密 硬件加密有问题，暂时不能使用
//******************************************************************
void AES_encrypt_hw(unsigned char* in, unsigned char* out, uint32_t* aes)
{
	...
	ECB_Encrypt_InputStruct.AESKeyMode = KEY_Mode_128bits;
	ECB_Encrypt_InputStruct.pKeyAddr = aes;
	ECB_Encrypt_InputStruct.pPtextAddr = in;
	ECB_Encrypt_InputStruct.LenPtext= 16;
	ECB_Encrypt_OutputStruct.pCtextAddr = out;

	/*!< ECB 加密	*/
	ECB_Encrypt(&ECB_Encrypt_InputStruct, &ECB_Encrypt_OutputStruct);
}

//******************************************************************
//! \brief  	AES_decrypt_hw
//! \param		in				密文数据
//! \param		out  			已解密明文
//! \param		aes				秘钥
//! \retval
...
void AES_decrypt_hw(unsigned char* in, unsigned char* out, uint32_t* aes)
{
	...
	/*解密输入结构体初始化*/
	ECB_Decrypt_InputStruct.AESKeyMode = KEY_Mode_128bits;
	ECB_Decrypt_InputStruct.pKeyAddr = aes;					//秘钥地址
	ECB_Decrypt_InputStruct.pCtextAddr = in;				//密文数据地址
	ECB_Decrypt_InputStruct.LenCtext= 16;					//密文长度
	ECB_Decrypt_OutputStruct.pPtextAddr = out;				//已解密明文地址
	//ECB 解密
	ECB_Decrypt(&ECB_Decrypt_InputStruct, &ECB_Decrypt_OutputStruct);
}
#endif
...
int EncrypData(uint8_t *in, uint8_t *out, uint16_t len, AES_KEY *pAesKey)
{
	...
	cnt = len/16;
	for (uint8_t i=0; i<cnt; i++) {
		AES_encrypt((void*)in, (void*)out, pAesKey);
        Feed_WDT();
		in += 16;
		out += 16;
	}
