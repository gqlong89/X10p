...
__IO uint8_t UART9_RxBuff[UART9_RX_SIZE] = {0};

UART_INFO_STR gUartPortAddr[SYS_UART_CNT] = {0};
void (*gUartInitFun[])(void) = {Init_Uart0,Init_Uart1,Init_Uart2,Init_Uart3,Init_Uart4,Init_Uart5,Init_Uart6,Init_Uart7,Init_Uart8,Init_Uart9};
...
int UsartPut(int portIndex, uint8_t data)
{
    UART_INFO_STR *pUart = &gUartPortAddr[portIndex];
    pUart->putCnt++;
    return FIFO_S_Put(&pUart->rxBuffCtrl, data);
}
...
{
    UART_INFO_STR *pUart = &gUartPortAddr[portIndex];
    return pUart->rxBuffCtrl.overFlowCnt;
}
...
{
    UART_INFO_STR *pUartInfo = &gUartPortAddr[0];
...
{
    UART_INFO_STR *pUartInfo = &gUartPortAddr[1];
	...
	HT_UART1->UARTCON = 0x000f;//0x005f;		//正逻辑,8位数据，偶校验，2停止位,接收发送使能
...
{
    UART_INFO_STR *pUartInfo = &gUartPortAddr[2];
...
{
    UART_INFO_STR *pUartInfo = &gUartPortAddr[3];
	...
	Set_Uart_Bandrate(HT_UART3, 4800);
...
{
    UART_INFO_STR *pUartInfo = &gUartPortAddr[4];
...
{
    UART_INFO_STR *pUartInfo = &gUartPortAddr[5];
...
{
    UART_INFO_STR *pUartInfo = &gUartPortAddr[6];
...
{
    UART_INFO_STR *pUartInfo = &gUartPortAddr[7];
...
{
    UART_INFO_STR *pUartInfo = &gUartPortAddr[8];
...
{
    UART_INFO_STR *pUartInfo = &gUartPortAddr[9];

    pUartInfo->index = 9;
    pUartInfo->type = SIMU_UART;
    FIFO_S_Init(&pUartInfo->rxBuffCtrl, (void*)UART9_RxBuff, sizeof(UART9_RxBuff));
    pUartInfo->Data.simUart.deviceIndex = 2;
}
